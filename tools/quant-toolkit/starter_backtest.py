...
def backtest(df: pd.DataFrame, fast: int, slow: int, fee_bps: float = 5.0) -> dict:
    ...
    def ann(scalar_ret: float) -> float:
        return (1 + scalar_ret) ** (252 / total_days) - 1
    strat_total = equity - 1
    vol = df["strategy_ret"].std() * np.sqrt(252)
    sharpe = ann(strat_total) / vol if vol else 0.0
    ...
    return {
        ...
        "strategy_ann_return": round(ann(strat_total), 4),
